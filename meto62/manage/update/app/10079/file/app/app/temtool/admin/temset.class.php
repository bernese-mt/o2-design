...
class temset extends admin {
	...
	public function dotable_temset_json() {
		...
		foreach($array as $key => $val) {
			$list = array();
			if(intval($val['type'])==1) $bigclass=$val['bigclass']?$val['bigclass']:$val['id'];
			$list[] = "<input name=\"id\" type=\"checkbox\" value=\"{$val[id]}\"/>\n<input name=\"bigclass-{$val['id']}\" type=\"hidden\" value=\"{$bigclass}\"/>";
			$list[] = '<i class="fa fa-caret-right"></i>';
			$list[] = $this->select($val['type'],$val[id]);
			$list[] = "<input type=\"text\" name=\"name-{$val[id]}\" class=\"ui-input\" placeholder=\"变量名\" data-norepeat='namenopt' data-required=\"1\" value=\"{$val['name']}\">";
	...
	public function dosetsave(){
		...
		foreach($list as $id){
			if($id){
				...
				if($type=='save' || !$type){
					...
					$bigclass  = $_M['form']['bigclass-'.$id] ? $_M['form']['bigclass-'.$id] : 0;
		...
		$tems = DB::get_all($query);
		foreach($tems as $keytems=>$valtems){
			$sql[] = "pos ='{$valtems['pos']}',no_order='{$valtems['no_order']}',type='{$valtems['type']}',style='{$valtems['style']}',selectd='{$valtems['selectd']}',name ='{$valtems['name']}',value='{$valtems['defaultvalue']}',defaultvalue='{$valtems['defaultvalue']}',valueinfo ='{$valtems['valueinfo']}',tips='{$valtems['tips']}',bigclass ='{$valtems['bigclass']}'";
		}
