...
class Admin extends Base {
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            if (!testWriteAble(DATA_PATH.'session')) {
                $this->error('請仔細檢查以下問題：<br/>1、磁盤空間大小是否100%；<br/>2、站點目錄權限是否爲755；<br/>3、站點目錄的權限，禁止用root:root ；<br/>4、如還沒解決，請點擊：<a href="http://www.eyoucms.com/wenda/6958.html" target="_blank">查看教程</a>');
            }
    ...
    public function admin_pwd()
    {
        ...
        if(IS_POST){
            ...
            if(!$admin || $admin['password'] != $enOldPwd){
                exit(json_encode(array('status'=>-1,'msg'=>'舊密碼不正确')));
    ...
    public function logout()
    {
        ...
        // session_destroy();
        session::clear();
        cookie('admin-treeClicked', null); // 清除并恢複欄目列表的展開方式
    ...
    public function admin_add()
    {
        ...
        if (IS_POST) {
            ...
            if (M('admin')->where("user_name", $data['user_name'])->count()) {
                $this->error("此用戶名已被注冊，請更換",url('Admin/admin_add'));
            } else {
    ...
    public function admin_del()
    {
        ...
        if (IS_POST) {
            ...
            if (!empty($id_arr)) {
                ...
                if($r){
                    adminLog('删除管理員：'.implode(',', $user_names));
                    $this->success('删除成功');
                }else{
                    $this->error('删除失敗');
                }
