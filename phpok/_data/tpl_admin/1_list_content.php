	...
	<div class="layui-card-body">
		...
		<div class="layui-form-item phpok-search">
			...
			<div class="layui-inline">
				...
				<div class="layui-input-block">
					<input type="text" name="keywords[user]" class="layui-input"<?php if($keywords){ ?> value="<?php echo $keywords['user'];?>"<?php } ?> placeholder="<?php echo P_Lang("会员账号、手机号及Email");?>" />
				</div>
			</div>
			<?php } ?>
			<div class="layui-inline">
			...
			<div class="layui-inline">
				...
				<div class="layui-input-block">
					<ul class="layout">
						<li class="layui-form-mid">
							<select name="keywords[_id]" lay-ignore>
								<option value="0"><?php echo P_Lang("等于");?> =</option>
								<option value="1"><?php echo P_Lang("大于");?> &gt;</option>
								<option value="2"><?php echo P_Lang("小于");?> &lt;</option>
							</select>
						</li>
						<li><input type="text" name="keywords[id]" value="<?php echo $keywords['id'];?>" /></li>
					</ul>
...
<div class="layui-card">
	...
	<div class="layui-card-body">
		...
		<tr>
			...
			<th width="20px">&nbsp;</th>
			<th>&nbsp;</th>
			<th><?php echo $rs['alias_title'] ? $rs['alias_title'] : P_Lang('主题');?></th>
			<?php if($rs['cate']){ ?>
		...
		<tr id="list_<?php echo $value['id'];?>" class="layui-tips" title="<?php echo $rs['alias_title'] ? $rs['alias_title'] : P_Lang('主题');?>: <?php echo $value['title'];?>&#10;<?php echo P_Lang("发布日期");?>: <?php echo date('Y-m-d H:i:s',$value['dateline']);?>">
			...
			<td>
				<div class="layui-btn-group">
					<?php if($rs['is_front']){ ?>
					<input type="button" value="<?php echo P_Lang("查看");?>" onclick="$.phpok.open('<?php echo $sys['www_file'];?>?id=<?php echo $value['id'];?>')" class="layui-btn layui-btn-sm layui-btn-normal" />
					<?php } ?>
					<?php if($popedom['modify']){ ?>
					<input type="button" value="<?php echo P_Lang("编辑");?>" onclick="top.$.win('<?php echo $rs['title'];?>_<?php echo P_Lang("编辑");?>_#<?php echo $value['id'];?>','<?php echo phpok_url(array('ctrl'=>'list','func'=>'edit','id'=>$value['id']));?>')" class="layui-btn layui-btn-sm" />
					<?php } ?>
					<?php if($popedom['delete']){ ?>
					<input type="button" value="<?php echo P_Lang("删除");?>" onclick="content_del('<?php echo $value['id'];?>')" class="layui-btn layui-btn-sm layui-btn-danger" />
					<?php } ?>
					<?php if($popedom['comment'] && ($rs['comment_status'] || $comments[$value['id']])){ ?>
					<button type="button"class="layui-btn layui-btn-sm" onclick="$.admin_list.reply_it('<?php echo $value['id'];?>')">
						<?php echo P_Lang("评论");?>
						<?php if($comments[$value['id']]['uncheck']){ ?><span class="layui-badge layui-bg-orange"><?php echo $comments[$value['id']]['uncheck'];?></span><?php } ?>
					</button>
					<?php } ?>
					<?php if($rs['subtopics'] && !$value['parent_id'] && $popedom['add']){ ?><input type="button" value="<?php echo P_Lang("加子主题");?>" onclick="$.win('<?php echo P_Lang("加子主题");?>_<?php echo $rs['title'];?>','<?php echo phpok_url(array('ctrl'=>'list','func'=>'edit','parent_id'=>$value['id'],'pid'=>$value['project_id']));?>')" class="layui-btn layui-btn-sm" /><?php } ?>
				</div>
			</td>
		</tr>
			...
			<tr id="list_<?php echo $vv['id'];?>" class="layui-tips" title="<?php echo $rs['alias_title'] ? $rs['alias_title'] : P_Lang('主题');?>: <?php echo $vv['title'];?>&#10;<?php echo P_Lang("发布日期");?>: <?php echo date('Y-m-d H:i:s',$vv['dateline']);?>">
				...
				<td>
					<div class="layui-btn-group">
						<?php if($rs['is_front']){ ?>
						<input type="button" value="<?php echo P_Lang("查看");?>" onclick="$.phpok.open('<?php echo $sys['www_file'];?>?id=<?php echo $vv['id'];?>')" class="layui-btn layui-btn-sm layui-btn-normal" />
						<?php } ?>
						<?php if($popedom['modify']){ ?><input type="button" value="<?php echo P_Lang("编辑");?>" onclick="top.$.win('<?php echo $rs['title'];?>_<?php echo P_Lang("编辑");?>_#<?php echo $vv['id'];?>','<?php echo phpok_url(array('ctrl'=>'list','func'=>'edit','id'=>$vv['id']));?>')" class="layui-btn layui-btn-sm" /><?php } ?>
						<?php if($popedom['delete']){ ?><input type="button" value="<?php echo P_Lang("删除");?>" onclick="content_del('<?php echo $vv['id'];?>')" class="layui-btn layui-btn-sm layui-btn-danger" /><?php } ?>
						<?php if($popedom['comment'] && ($rs['comment_status'] || $comments[$vv['id']])){ ?>
						<button type="button"class="layui-btn layui-btn-sm" onclick="$.admin_list.reply_it('<?php echo $vv['id'];?>')">
							<?php echo P_Lang("评论");?>
