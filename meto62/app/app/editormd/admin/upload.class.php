...
class upload extends admin {
	...
	function __construct($savePath, $saveURL, $formats, $randomNameType = 1, $randomLength = '', $cover = true, $maxSize = 1024){
            global $_M;
            parent::__construct(); 
	...
	public function doeditor(){
        ...
        if(!file_exists($savePath)){
           ...
           { 
            $this->doconfig(array(
            'maxSize' => $_M[config][met_file_maxsize]*1024,//允许上传的最大文件大小，以KB为单位，默认值为1024
            'cover'   => true         // 是否覆盖同名文件，默认为true
           ));
 
           if($this->doupload($savePath,$saveURL,$name))
           {
            $this->domessage('上传成功！', 1);

           }else{
            $this->domessage('上传失败！', 0);
        }
    ...
    public function doconfig($configs) 
        {
        ...
        public function doupload($savePath,$saveURL,$name)
        {         
            if(empty($_FILES[$name]['name'])) //上传文件为空时
            {
               $this->domessage($this->errors['empty']);
            ...
            if(!file_exists($savePath)) //目录不存在
            {


                $this->domessage($this->errors['not_exist']);
                
                return false;
            }

            if(!is_writable($savePath)) //目录不可写
            {
                $this->domessage($this->errors['unwritable']);
                
                return false;
            }
       
            $this->fileExt  = $this->dogetFileExt($this->files["name"]); //取得扩展名
            //$this->savePath =$savePath;

            $this->dosetSeveName($savePath,$saveURL);
            
            return $this->domoveFile($savePath);
        }
        ...
        private function domoveFile($savePath)
        {
            $files = $this->files;
            ...
            if (!$this->cover) //当不能覆盖时
            {
                ...
                {
                    $this->domessage($this->saveName . $this->errors['same_file']);
            ...
            if (!@move_uploaded_file($files["tmp_name"], iconv("utf-8", "gbk",$this->savePath)))
            {          
                switch($files["error"])
                {
                    case '0':
        ...
        private function dorandomFileName($savePath)
        {
            ...
            {   
                ...
                $this->savePath=$savePath. $fileName.".".$this->fileExt;
         ...
         */
         ...
         public function dosetSeveName($savePath,$saveURL)
         {           

            $this->saveName = $saveURL.$this->dorandomFileName($savePath).".".$this->fileExt;
         ...
         */
         ...
         public function dogetFileExt($fileName)
         {
        ...
        public function domessage($message, $success = 0)
        {
            $array = array(
                'success' => $success
            );
 ...
 //$imageUploader = new index($savePath, $saveURL, $formats['image'], true); 
