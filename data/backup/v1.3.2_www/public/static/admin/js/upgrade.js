...
function checkdir(obj) {
    ...
    $.ajax({
        ...
        error: function(request) {
            parent.layer.closeAll();
            parent.layer.alert("检测不通过，可能被服务器防火墙拦截，请添加白名单，或者联系技术协助！", {icon: 2}, function(){
                top.location.reload();
            });
        },
        success: function(res) {
            ...
            if (1 == res.code) {
                ...
                //提示框
                if (2 == res.data.code) {
                    var alert = parent.layer.alert(res.msg, {icon: 2});
                } else {
...
function upgrade(obj){
    ...
    $.ajax({
        ...
        error: function(request) {
            parent.layer.closeAll();
            parent.layer.alert("升级失败，请第一时间联系技术协助！", {icon: 2}, function(){
                top.location.reload();
            });
        },
        success: function(res) {
            ...
            if(1 == res.code){
                ...
                parent.layer.closeAll();
                parent.layer.alert(res.msg, {icon: 2}, function(){
                    top.location.reload();
                });
            }
        }
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if(res.status){
                    ...
                    parent.layer.closeAll();
                    var _parent = parent;
                    _parent.layer.alert('已升级最新版本，自动备份数据库失败，请立即前往备份！', {icon: 1}, function(){
                        var url = eyou_basefile + "?m="+module_name+"&c=Tools&a=index";
